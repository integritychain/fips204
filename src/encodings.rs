...
pub(crate) fn pk_encode<const K: usize, const PK_LEN: usize>(
    p: &[u8; 32], t1: &[R; K],
        ...
        t1.iter().all(|t| is_in_range(t, 0, (1 << (bit_length(Q - 1) as u32 - D)) - 1)),
...
pub(crate) fn sk_encode<const K: usize, const L: usize, const SK_LEN: usize>(
    ...
    // note: lower is_in_range is a positive number, that is converted to negative
        ...
        t0.iter().all(|x| is_in_range(x, (1 << (D - 1)) - 1, 1 << (D - 1))),
...
pub(crate) fn sk_decode<const K: usize, const L: usize, const SK_LEN: usize>(
    ...
    //return Ok((rho, k, tr, [[0i32; 256]; L], [[0i32; 256]; K], [[0i32; 256]; K]));
    ...
    //return Ok((rho, k, tr, s1, s2, [[0i32; 256]; K]));
    ...
    let t0_ok = t0.iter().all(|r| is_in_range(r, (1 << (D - 1)) - 1, 1 << (D - 1)));
