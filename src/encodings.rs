...
///
/// # Errors
/// Returns an error when and element of t1 is out of range
...
///
/// # Errors
/// Returns an error when any of the input coefficients are out of range
...
///
/// # Errors
/// Returns an error when any of the output coefficients are out of range
...
pub(crate) fn sig_encode<
    ...
    const SIG_LEN: usize,
>(
    gamma1: i32, omega: i32, c_tilde: &[u8], z: &[R; L], h: &[R; K],
) -> Result<[u8; SIG_LEN], &'static str> {
    let mut sigma = [0u8; SIG_LEN];
    ensure!(c_tilde.len() == LAMBDA_DIV4, "Algorithm 20: {line!()}");
    let bl = bit_length(gamma1 - 1);
    ensure!(
        sigma.len() == LAMBDA_DIV4 + L * 32 * (1 + bl) + omega as usize + K,
...
mod tests {
    ...
    fn test_sig_roundtrip() {
        ...
        let sigma = sig_encode::<4, 4, {128/4}, 2420>(2i32.pow(17), 80, &c_tilde, &z, &h).unwrap();
        // let mut c_test = [0u8; 2 * 128 / 8];
        // let mut z_test = [[0i32; 256]; 4];
        // let mut h_test = [[0i32; 256]; 4];
        let (c_test, z_test, h_test) = sig_decode::<4, 4, {128/4}>(2i32.pow(17), 80, &sigma).unwrap();
        //        assert!(res.is_ok());
