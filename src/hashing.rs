...
pub(crate) fn sample_in_ball(tau: i32, rho: &[u8; 32]) -> R {
...
pub(crate) fn rej_ntt_poly_vartime(rhos: &[&[u8]]) -> T {
    ...
    while j < 256 {
        ...
        let a_hat_j = coef_from_three_bytes_vartime(h128pc); // gets a result
...
pub(crate) fn rej_bounded_poly_vartime(eta: i32, rhos: &[&[u8]]) -> R {
    ...
    while j < 256 {
        ...
        let z0 = coef_from_half_byte_vartime(eta, z[0] & 0x0f);

        // 6: z1 ← CoefFromHalfByte(⌊z/16⌋, η)
        let z1 = coef_from_half_byte_vartime(eta, z[0] >> 4);
