...
pub(crate) fn sample_in_ball(tau: i32, rho: &[u8; 32]) -> Result<R, &'static str> {
    ...
    // 12: return c
    ensure!(c.iter().filter(|e| e.abs() != 0).count() == tau, "Alg23: bad hamming weight");
    Ok(c)
}
...
pub(crate) fn rej_ntt_poly_vartime(rhos: &[&[u8]]) -> T {
    ...
    while j < 256 {
        ...
        let a_hat_j = conversion::coef_from_three_bytes_vartime(h128pc); // gets a result
...
pub(crate) fn rej_bounded_poly_vartime(eta: i32, rhos: &[&[u8]]) -> R {
    ...
    while j < 256 {
        ...
        let z0 = conversion::coef_from_half_byte_vartime(eta, z[0] & 0x0f);

        // 6: z1 ← CoefFromHalfByte(⌊z/16⌋, η)
        let z1 = conversion::coef_from_half_byte_vartime(eta, z[0] >> 4);
...
pub(crate) fn expand_a_vartime<const K: usize, const L: usize>(rho: &[u8; 32]) -> [[T; L]; K] {
    ...
    for (a_row, r) in cap_a_hat.iter_mut().zip(0..u8::try_from(K).unwrap()) {
        ...
        for (a_element, s) in a_row.iter_mut().zip(0..u8::try_from(L).unwrap()) {
            ...
            *a_element = rej_ntt_poly_vartime(&[&rho[..], &[s], &[r]]);
...
pub(crate) fn expand_s_vartime<const K: usize, const L: usize>(
    ...
    for (s1r, r) in s1.iter_mut().zip(0..u8::try_from(L).unwrap()) {
        ...
        *s1r = rej_bounded_poly_vartime(eta, &[rho, &[r], &[0]]);
    ...
    for (s2r, r) in s2.iter_mut().zip(0..u8::try_from(K).unwrap()) {
        ...
        ensure!((r as usize + L) < 255, "Alg27: r + L out of range u8");
        *s2r = rej_bounded_poly_vartime(eta, &[rho, &[r + u8::try_from(L).unwrap()], &[0]]);

        // 6: end for
    }

    // 7: return (s_1 , s_2)
    ensure!(s1.iter().all(|r| is_in_range(r, eta, eta)), "Alg27: s1 out of range");
    ensure!(s2.iter().all(|r| is_in_range(r, eta, eta)), "Alg27: s2 out of range");
    Ok((s1, s2))
}
...
/// Returns an error on out of range s coefficients.
/// Propagates any errors generated by called functions.
