...
>(
    ...
    let PrivateKey { rho, cap_k, tr, s_hat_1_mont, s_hat_2_mont, t_hat_0_mont } = esk;
    //
    // 2: s_hat_1 ← NTT(s_1)
    ...
    loop {
        ...
        if !CTEST && ((z_norm >= (gamma1 - beta)) || (r0_norm >= (gamma2 - beta))) {
            kappa_ctr += u16::try_from(L).expect("cannot fail; K is static parameter");
            continue;
            //
...
>(
    ...
    debug_assert!(infinity_norm(&z) <= gamma1, "Alg 3: i_norm out of range"); // Fuzz target

    // 5: cap_a_hat ← ExpandA(ρ)    ▷ A is generated and stored in NTT representation as cap_A_hat
    // stashed in step 9 below to minimize stack consumption when dev has opt-level = 0
...
pub(crate) fn private_to_public_key<const K: usize, const L: usize>(
    ...
    let PrivateKey { rho, cap_k: _, tr, s_hat_1_mont, s_hat_2_mont, t_hat_0_mont } = sk;
    let cap_a_hat: [[T; L]; K] = expand_a::<false, K, L>(rho);

    // mont->norm elements to recover s_1_hat
    let s_1_hat: [T; L] = core::array::from_fn(|l|
        T(core::array::from_fn(|n| mont_reduce(i64::from(s_hat_1_mont[l].0[n])))));

    // mont->norm each n coeff, of L entries of T, then inverse NTT into R
    let s_2: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| mont_reduce(i64::from(s_hat_2_mont[k].0[n]))))
    }));
    ...
    let t_0: [R; K] = inv_ntt(&core::array::from_fn(|k| {
        T(core::array::from_fn(|n| mont_reduce(i64::from(t_hat_0_mont[k].0[n]))))
    }));
